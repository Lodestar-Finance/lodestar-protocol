...
describe('ComptrollerV1', function() {
  ...
  beforeEach(async () => {
    ...
    brains = await deploy('ComptrollerV1');
    unitroller = await deploy('Unitroller');
    unitrollerProxy = await saddle.getContractAt('Comptroller', unitroller._address);
  });

  let initializeBrains = async (priceOracle, closeFactor) => {
    await send(unitroller, '_setPendingImplementation', [brains._address]);
    // console.log('yo', unitroller._address, priceOracle._address, closeFactor)
    await send(brains, '_become', [unitroller._address]);
    //let unitroller1 = await saddle.getContractAt('Comptroller', unitroller.address);
    await send(unitrollerProxy, '_setPriceOracle', [priceOracle._address]);
    await send(unitrollerProxy, '_setCloseFactor', [closeFactor]);
  ...
  describe('delegating to comptroller v1', () => {
    const closeFactor = "51000000000000000";
    let unitrollerAsComptroller, cToken;

    beforeEach(async () => {
      unitrollerAsComptroller = await initializeBrains(oracle, etherMantissa(0.06));
