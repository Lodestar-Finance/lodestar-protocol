const { makeComptroller, makeCToken, enterMarkets, quickMint } = require("../Utils/Compound");
...
describe("Comptroller", () => {
    ...
    describe("liquidity", () => {
        ...
        it("allows a borrow up to collateralFactor, but not more", async () => {
            ...
            ({ 1: liquidity, 2: shortfall } = await call(cToken.comptroller, "getHypotheticalAccountLiquidity", [
                user,
                cToken._address,
                0,
                amount
        ...
        it("allows entering 3 markets, supplying to 2 and borrowing up to collateralFactor in the 3rd", async () => {
            ...
            const cToken2 = await makeCToken({
                supportMarket: true,
                comptroller: cToken1.comptroller,
                collateralFactor: cf2,
                underlyingPrice: up2
            });
            const cToken3 = await makeCToken({
                supportMarket: true,
                comptroller: cToken1.comptroller,
            ...
            ({ 1: liquidity, 2: shortfall } = await call(cToken3.comptroller, "getHypotheticalAccountLiquidity", [
                user,
                cToken3._address,
                Math.floor(c2),
                0
            ...
            ({ 1: liquidity, 2: shortfall } = await call(cToken1.comptroller, "getHypotheticalAccountLiquidity", [
                user,
                cToken1._address,
                amount1,
                0
    ...
    describe("getAccountLiquidity", () => {
        it("returns 0 if not 'in' any markets", async () => {
            ...
            const { 0: error, 1: liquidity, 2: shortfall } = await call(comptroller, "getAccountLiquidity", [
                accounts[0]
            ]);
            expect(error).toEqualNumber(0);
            expect(liquidity).toEqualNumber(0);
    ...
    describe("getHypotheticalAccountLiquidity", () => {
        it("returns 0 if not 'in' any markets", async () => {
            ...
            const { 0: error, 1: liquidity, 2: shortfall } = await call(
                cToken.comptroller,
                "getHypotheticalAccountLiquidity",
                [accounts[0], cToken._address, 0, 0]
            );
            expect(error).toEqualNumber(0);
            expect(liquidity).toEqualNumber(0);
            expect(shortfall).toEqualNumber(0);
