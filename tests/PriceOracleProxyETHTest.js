...
describe("PriceOracleProxyETH", () => {
    ...
    beforeEach(async () => {
        ...
        oracle = await deploy("PriceOracleProxyETH", [
            root,
            backingOracle._address,
            mockEthUsdAggregator._address,
            cEth._address,
            plvGLP._address,
            glpOracle._address
        ]);
    ...
    describe("getUnderlyingPrice", () => {
        let setPrice = async (cToken, price, base) => {
            ...
            await send(oracle, "_setAggregators", [[cToken._address], [mockAggregator._address], [base]]);
        };
        ...
        let setAndVerifyBackingPrice = async (cToken, price) => {
            await send(backingOracle, "setUnderlyingPrice", [cToken._address, etherMantissa(price)]);

            let backingOraclePrice = await call(backingOracle, "assetPrices", [cToken.underlying._address]);
        ...
        it("returns correctly for ETH base aggregator tokens", async () => {
            ...
            console.log(
                "price & calculated price here",
                price,
                etherMantissa(price * 10 ** (18 - underlyingDecimals)).toFixed()
            );
        ...
        it("returns correctly for USD base aggregator tokens", async () => {
            ...
            expect(proxyPrice).toEqual(
                BigInt(
                    (price * 10 ** (18 - aggregatorDecimals) * 10 ** (18 - underlyingDecimals)) /
                        (ethUsdPrice / 10 ** aggregatorDecimals)
                ).toString()
            );
    ...
    describe("TWAP", () => {
        it("sets anchor successfully", async () => {
    ...
    describe("_setAdmin", () => {
        ...
        it("fails to set admin for non-admin", async () => {
            await expect(send(oracle, "_setAdmin", [accounts[0]], { from: accounts[0] })).rejects.toRevert(
                "revert only the admin may set new admin"
            );
        });
    });
    ...
    describe("_setGuardian", () => {
        ...
        it("fails to set guardian for non-admin", async () => {
            await expect(send(oracle, "_setGuardian", [accounts[0]], { from: accounts[0] })).rejects.toRevert(
                "revert only the admin may set new guardian"
            );
        });
    });
    ...
    describe("_setAggregators", () => {
        ...
        it("set aggregators successfully", async () => {
            expect(
                await send(oracle, "_setAggregators", [[cOthers._address], [mockAggregator._address], [0]])
            ).toSucceed(); // 0: USD
        });

        it("fails to set aggregators for non-admin", async () => {
            await expect(
                send(oracle, "_setAggregators", [[cOthers._address], [mockAggregator._address], [0]], {
                    from: accounts[0]
                })
            ).rejects.toRevert("revert only the admin or guardian may set the aggregators"); // 0: USD
            expect(await send(oracle, "_setGuardian", [accounts[0]])).toSucceed();
            await expect(
                send(oracle, "_setAggregators", [[cOthers._address], [mockAggregator._address], [0]], {
                    from: accounts[0]
                })
            ).rejects.toRevert("revert guardian may only clear the aggregator"); // 0: USD
        });

        it("fails to set aggregators for mismatched data", async () => {
            await expect(send(oracle, "_setAggregators", [[cOthers._address], [], [0]])).rejects.toRevert(
                "revert mismatched data"
            ); // 0: USD
            await expect(
                send(oracle, "_setAggregators", [[cOthers._address], [mockAggregator._address], []])
            ).rejects.toRevert("revert mismatched data"); // 0: USD
            await expect(send(oracle, "_setAggregators", [[], [mockAggregator._address], [0]])).rejects.toRevert(
                "revert mismatched data"
            ); // 0: USD
        });
        ...
        it("clear aggregators successfully", async () => {
            expect(await send(oracle, "_setGuardian", [accounts[0]])).toSucceed();
            expect(
                await send(oracle, "_setAggregators", [[cOthers._address], [address(0)], [0]], { from: accounts[0] })
            ).toSucceed(); // 0: USD
        });
    });
});
