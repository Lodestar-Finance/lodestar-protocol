...
async function makeGLPPriceOracle(root) {
    // create mock contracts
    const mockGLPAddress = await deploy('MockGLP');
    const mockplvGLPAddress = await deploy('MockPlvGLP');

    // create GLP oracle and feed in above mock data
    return await deploy('GLPOracle', [root, mockGLPAddress._address, mockGLPAddress._address, mockplvGLPAddress._address]);
}
...
async function makeMockUSDAggregator(opts = {}) {
  const answer = dfn(opts.answer, 100000000);
  return await deploy('MockUSDAggregator', [answer]);
}
